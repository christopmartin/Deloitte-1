...
const SN_CATALOG = [
  ...
  { table: 'sys_ui_policy',      captureType: 'ui_policy',      wbDesignType: 'form_design',    mappingStatus: 'partial',  tier: 'C', partialIntent: 'by-intent',
    fields: ['short_description', 'table', 'conditions'], complexityWeight: 1,
    featureNote: 'UI policies — form behaviour authored in ServiceNow; condition + linked policy actions (mandatory/read-only per field) now captured for fidelity, not just drift' },
  ...
  { table: 'sc_cat_item',        captureType: 'catalog_item',   wbDesignType: 'catalog_item',   mappingStatus: 'mapped',
    fields: ['name', 'short_description', 'category', 'workflow'], complexityWeight: 1,
    featureNote: 'Service catalog items (category + fulfillment workflow captured; variables via item_option_new child capture)' },
