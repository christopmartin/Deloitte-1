...
function buildSystemPrompt(doc, threshold, answeredClarifications, existingSummary, bestPractices) {
  ...
  lines.push(
    ...
    `  form_design       — any form / screen / view layout: section order, field order, mandatory &`,
    `                      read-only fields, and dynamic UI behaviour`,
    ...
    `  3. data_model FIRST, then form_design, then business_logic — the ServiceNow platform layer.`,
    `     A form lays out a table, so extract the data_model first and set the form's data_model_name`,
    `     to it. business_logic usually runs on a table too — set its data_model_name when known.`,
