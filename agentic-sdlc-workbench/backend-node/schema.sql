...
CREATE TABLE IF NOT EXISTS asdlc_use_case (
    ...
    best_practice_ref        TEXT,                                  -- verified [BP-xxx] slug, when system_generated cites a specific house rule
    visibility_scope         TEXT NOT NULL DEFAULT 'PROJECT',
...
-- SN metadata type is ever un-representable or dropped. The 5 Level-1 SN-technical
-- tables above (data_model/form_design/business_logic/catalog_item/integration) and
-- the 22-type business-design registry remain unchanged; SN-sourced rows here can
-- PROJECT onto an L1 row (Tier A) via projected_entity_type/_id, or stand alone
-- (Tier B/C). Deploy-back goes through a dedicated Fluent constructor (typed), the
-- SDK's generic Record() (record), or Record()+$override (override).
--
-- Recursive: parent-child SN structures (Table→Column, CatalogItem→Variable,
-- Flow→Action, Form→section) are modelled as child rows via parent_artifact_id, so
-- each child keeps its own sys_id identity and round-trips/drifts independently.
CREATE TABLE IF NOT EXISTS asdlc_sn_artifact (
    sn_artifact_id    TEXT PRIMARY KEY,
    project_id        TEXT REFERENCES asdlc_project(project_id),
    slug              TEXT,                          -- ART-### per project (optional)
    -- ── Type identity (drives deploy) ──
    sn_metadata_type  TEXT NOT NULL,                 -- registry key: 'acl','sla','sp_widget','table'
    fluent_api_name   TEXT,                          -- resolved Fluent constructor: 'Acl','Sla','SPWidget'
    deploy_strategy   TEXT NOT NULL DEFAULT 'record'
        CHECK (deploy_strategy IN ('typed','record','override')),
    tier              TEXT NOT NULL DEFAULT 'C' CHECK (tier IN ('A','B','C')),
    name              TEXT NOT NULL DEFAULT '(unnamed)',
    -- ── Structured round-trip body ──
    payload           TEXT NOT NULL DEFAULT '{}',    -- JSON: typed-API field map
    override_fields   TEXT NOT NULL DEFAULT '{}',    -- JSON: x_/u_ + fields the typed API can't express ($override)
    -- ── Recursive children ──
    parent_artifact_id TEXT REFERENCES asdlc_sn_artifact(sn_artifact_id),
    child_role        TEXT,                          -- 'column','variable','section','action'
    child_order       INTEGER,
    -- ── Projection link to the clean Level-1 row this surfaces as (nullable) ──
    projected_entity_type TEXT,                      -- 'data_model'|'business_logic'|NULL (long tail)
    projected_entity_id   TEXT,
    -- ── Level-2 provenance (hidden; identical semantics to the L1 tables) ──
    source_system     TEXT NOT NULL DEFAULT 'servicenow',
    source_sys_id     TEXT,                          -- round-trip identity; PATCH vs POST
    source_table      TEXT,
    source_scope      TEXT,
    source_fluent     TEXT,                          -- verbatim Fluent snippet
    source_hash       TEXT,                          -- content hash for tier-0 drift detection
    sdk_version       TEXT,                          -- SDK capability surface this row was last deployed against
    visibility_scope  TEXT NOT NULL DEFAULT 'PROJECT',
    lifecycle_status  TEXT NOT NULL DEFAULT 'active',
    created_by        TEXT,
    created_at        TEXT NOT NULL DEFAULT (datetime('now')),
...
CREATE INDEX IF NOT EXISTS idx_sn_artifact_project  ON asdlc_sn_artifact(project_id);
CREATE INDEX IF NOT EXISTS idx_sn_artifact_sysid    ON asdlc_sn_artifact(source_sys_id);
CREATE INDEX IF NOT EXISTS idx_sn_artifact_parent   ON asdlc_sn_artifact(parent_artifact_id);
CREATE INDEX IF NOT EXISTS idx_sn_artifact_type     ON asdlc_sn_artifact(project_id, sn_metadata_type);
CREATE INDEX IF NOT EXISTS idx_sn_artifact_proj_ent ON asdlc_sn_artifact(projected_entity_type, projected_entity_id);

-- SDK capability registry — a DERIVED snapshot of the installed now-sdk / Fluent
-- surface, regenerated by agent/sn-capability-sync.js from `npx @servicenow/sdk
-- explain --list`. NOT hand-maintained source: adding/upgrading a type is a re-scan,
-- not a code edit. One row per first-class metadata type the SDK can deploy.
--   deploy_strategy 'typed'  → a dedicated Fluent constructor exists (fluent_api_name)
--                   'record' → no dedicated API; deploy via the generic Record() fallback
--   tier            'A' projects onto an existing L1 table; 'B' curated field_schema/editor; 'C' long tail
CREATE TABLE IF NOT EXISTS asdlc_sn_type_registry (
    sn_metadata_type TEXT PRIMARY KEY,               -- 'acl','sla','table','sp_widget'
    fluent_api_name  TEXT,                           -- 'Acl','Sla','Table','SPWidget' (NULL ⇒ record fallback)
    source_table     TEXT,                           -- backing SN table, e.g. 'sys_security_acl'
    explain_topic    TEXT,                            -- now-sdk explain topic, e.g. 'acl-api'
    deploy_strategy  TEXT NOT NULL DEFAULT 'record'
        CHECK (deploy_strategy IN ('typed','record','override')),
    tier             TEXT NOT NULL DEFAULT 'C' CHECK (tier IN ('A','B','C')),
    projected_entity_type TEXT,                       -- L1 entity_type for Tier-A projection (nullable)
    parent_type      TEXT,                            -- parent sn_metadata_type for sub-elements (nullable)
    child_role       TEXT,                            -- role under the parent ('column','variable',…)
    field_schema     TEXT,                            -- JSON descriptor for the Tier-B editor (nullable)
    tags             TEXT NOT NULL DEFAULT '[]',       -- JSON array of catalog tags (search/aliases)
    sdk_version      TEXT,                            -- SDK version this row was discovered from
    curated          INTEGER NOT NULL DEFAULT 0,       -- 1 = human-curated fields a re-scan must not clobber
    discovered_at    TEXT NOT NULL DEFAULT (datetime('now')),
    updated_at       TEXT NOT NULL DEFAULT (datetime('now'))
);
CREATE INDEX IF NOT EXISTS idx_sn_type_registry_table  ON asdlc_sn_type_registry(source_table);
CREATE INDEX IF NOT EXISTS idx_sn_type_registry_parent ON asdlc_sn_type_registry(parent_type);
...
CREATE TABLE IF NOT EXISTS asdlc_best_practice (
    ...
    slug             TEXT,                             -- short citable id (BP-001, ...) — what the AI cites, never the UUID
